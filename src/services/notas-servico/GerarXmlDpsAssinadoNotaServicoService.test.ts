...
function criarService(
  ...
  const notaRepository: NotaServicoRepository = {
    ...
    buscarPorIdEEmpresaId: vi.fn().mockResolvedValue(
      new NotaServico({
        ...
        ambienteFiscal: props?.ambienteFiscal ?? AmbienteFiscal.HOMOLOGACAO,
      }),
    ),
  };
  ...
  return new GerarXmlDpsAssinadoNotaServicoService(
    ...
    provedor,
    assinador,
    criarResolverConfiguracaoFiscalPadrao(),
    notaRepository,
  );
}

function criarResolverConfiguracaoFiscalPadrao() {
  return {
    obterCertificadoA1ParaAmbiente: vi.fn(
      async (_empresaId: string, ambienteFiscal: AmbienteFiscal) => {
