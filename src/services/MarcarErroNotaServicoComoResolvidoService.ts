...
export class MarcarErroNotaServicoComoResolvidoService {
  constructor(private readonly notaRepository: NotaServicoRepository) {}

  async executar(autenticacao: TokenPayload, notaId: string): Promise<NotaServico> {
    ...
    if (nota.status !== StatusNota.ERRO) {
      throw new TransicaoStatusNotaInvalidaError(
        'Somente uma nota com erro pode ser marcada como resolvida.',
      );
    }
